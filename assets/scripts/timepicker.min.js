!(function () {
  ...
  function s(s, c) {
      ...
      if (!i || !(u < b - w || u > b + w)) {
        ...
        p && h.append(V.canvas),
          ...
          a.off(m).on(m, function (t) {
            ...
            (i || k) && s === r && p === l && V.setHand(s, p),
              "hours" === V.currentView
                ? V.toggleView("minutes", M / 2)
                : c.autoclose &&
                  (V.minutesView.addClass("clockpicker-dial-out"),
                  setTimeout(function () {
                    V.done();
                  }, M / 2)),
              h.prepend(X),
              clearTimeout(v),
              V.popover.removeClass("clockpicker-moving"),
              a.off(d);
          });
      }
    }
      ...
      c.ampmclickable
        ? (this.spanAmPm.empty(),
          n('<div id="click-am">AM</div>')
            .on("click", function () {
              V.spanAmPm.children("#click-am").addClass("text-primary"),
                V.spanAmPm.children("#click-pm").removeClass("text-primary"),
                (V.amOrPm = "AM");
            })
            .appendTo(this.spanAmPm),
          n('<div id="click-pm">PM</div>')
            .on("click", function () {
              V.spanAmPm.children("#click-pm").addClass("text-primary"),
                V.spanAmPm.children("#click-am").removeClass("text-primary"),
                (V.amOrPm = "PM");
            })
            .appendTo(this.spanAmPm))
        : (n(
            ...
            .on("click", function () {
              (V.amOrPm = "AM"),
                V.amPmBlock.children(".pm-button").removeClass("active"),
                V.amPmBlock.children(".am-button").addClass("active"),
                V.spanAmPm.empty().append("AM");
            })
          ...
          n(
            ...
            .on("click", function () {
              (V.amOrPm = "PM"),
                V.amPmBlock.children(".am-button").removeClass("active"),
                V.amPmBlock.children(".pm-button").addClass("active"),
                V.spanAmPm.empty().append("PM");
            })
            .appendTo(this.amPmBlock));
    }
    T.attr("type", "text");
    var D = (
        (this.options["default"] || this.input.prop("value") || "now") + ""
      ).split(":"),
      S = (
        (this.options["default"] || this.input.prop("value") || "now") + ""
      ).split(":");
    ...
    if ("now" === D[0]) {
      ...
      var B = new Date(+new Date() + this.options.fromnow),
        ...
        j = B.getMinutes();
      (D = c.twelvehour
        ? [i(0 == I ? 12 : I > 12 ? I - 12 : I), i(j) + (I < 12 ? "AM" : "PM")]
        : [i(I), i(j)]),
        (S = [i(I), i(j), "00"]),
        T.prop({ default: S.join(":") })
          .data({ default: S.join(":"), submit: S.join(":") })
          .attr({ "data-default": S.join(":"), "data-submit": S.join(":") });
    } else
      T.prop({ value: D[0] + ":" + D[1] })
        .data({ submit: S.join(":") })
        .attr({ value: D[0] + ":" + D[1], "data-submit": S.join(":") });
    c.darktheme && l.addClass("darktheme"),
      n(
        '<button type="button" class="btn-flat clockpicker-button" tabindex="' +
          (c.twelvehour ? "4" : "2") +
          '">' +
          c.canceltext +
          "</button>",
      )
        .click(n.proxy(this.cancelInput, this))
      ...
      T.on("focus.clockpicker click.clockpicker", n.proxy(this.show, this));
    var E,
      O,
      z,
      U,
      L = n('<div class="clockpicker-tick"></div>');
    if (c.twelvehour)
      for (E = 1; E < 13; E += 1)
        (O = L.clone()),
          (z = (E / 6) * Math.PI),
    ...
    for (E = 0; E < 60; E += 5)
      (O = L.clone()),
        (z = (E / 30) * Math.PI),
        O.css({ left: f + Math.sin(z) * b - w, top: f - Math.cos(z) * b - w }),
        O.css("font-size", "140%"),
        O.html(i(E)),
        P.append(O),
        O.on(u, r);
    if (
      ...
      var X = l.find(".clockpicker-canvas"),
        Y = t("svg");
      Y.setAttribute("class", "clockpicker-svg"),
        Y.setAttribute("width", y),
        Y.setAttribute("height", y);
      var q = t("g");
  ...
  var n = window.jQuery,
    ...
    p =
      ...
      (function () {
        ...
        return (
          (i.innerHTML = "<svg/>"),
          (t = (i.firstChild && i.firstChild.namespaceURI) == r),
          (i.innerHTML = ""),
          t
        );
      })(),
  ...
  (s.DEFAULTS = {
    ...
    (s.prototype.show = function (t) {
      if (
        ((this.setAMorPM = function (t) {
          var i = t,
            e = "pm" == t ? "am" : "pm";
          this.options.twelvehour &&
            ((this.amOrPm = i.toUpperCase()),
            this.options.ampmclickable
              ? (this.spanAmPm.children("#click-" + i).addClass("text-primary"),
                this.spanAmPm
                  .children("#click-" + e)
              ...
              : (this.amPmBlock
                  .children("." + e + "-button")
                  .removeClass("active"),
                this.amPmBlock.children("." + i + "-button").addClass("active"),
        ...
        this.input.blur(),
          ...
          this.isAppended ||
            (this.options.hasOwnProperty("container")
              ? this.popover.appendTo(this.options.container)
              : this.popover.insertAfter(this.input),
            this.setAMorPM("pm"),
            c.on("resize.clockpicker" + this.id, function () {
              e.isShown && e.locate();
            }),
            (this.isAppended = !0));
        var s = (
        ...
        if (
          (this.options.twelvehour &&
            ...
            (s[1].includes("AM") || parseInt(s[0]) < 12
              ? this.setAMorPM("am")
              : this.setAMorPM("pm"),
            (s[1] = s[1].replace("AM", "").replace("PM", ""))),
          "now" === s[0])
        ) {
          var r = new Date(+new Date() + this.options.fromnow);
          r.getHours() >= 12 ? this.setAMorPM("pm") : this.setAMorPM("am"),
            (s = [r.getHours(), r.getMinutes()]);
        }
        (this.hours = +s[0] || 0),
          (this.minutes = +s[1] || 0),
    ...
    (s.prototype.setHand = function (t, e, s, o) {
      ...
      if (
        ...
        (this[this.currentView] = c),
        this[r ? "spanHours" : "spanMinutes"].html(i(c)),
        !p)
      )
        return void this[r ? "hoursView" : "minutesView"]
      ...
      this.hand.setAttribute("x2", v),
        this.hand.setAttribute("y2", f),
        this.bg.setAttribute("cx", y),
        this.bg.setAttribute("cy", M),
        this.fg.setAttribute("cx", y),
        this.fg.setAttribute("cy", M);
    }),
    (s.prototype.cancelInput = function () {
    ...
    (s.prototype.done = function () {
      ...
      (submit = i(this.hours) + ":" + i(this.minutes) + ":00"),
        this.options.twelvehour &&
          ((e += this.amOrPm),
          "PM" == this.amOrPm
            ? (submit =
                (this.hours < 12 ? this.hours + 12 : 12) +
                ":" +
                i(this.minutes) +
                ":00")
            : (submit =
                (this.hours < 12 ? i(this.hours) : "00") +
                ":" +
                i(this.minutes) +
                ":00")),
        this.input
          .prop({ value: e })
          .data({ submit: submit })
          .attr({ value: e, "data-submit": submit }),
        ...
        this.options.afterDone &&
          "function" == typeof this.options.afterDone &&
          this.options.afterDone(this.input, submit);
    }),
    (s.prototype.remove = function () {
      this.element.removeData("clockpicker"),
        ...
        this.isAppended &&
          (c.off("resize.clockpicker" + this.id), this.popover.remove());
    }),
